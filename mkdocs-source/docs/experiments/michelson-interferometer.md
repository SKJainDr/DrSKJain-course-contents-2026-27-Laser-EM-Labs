...
!!! tip "Try it live"
    Explore this on the interactive simulator with real-time sliders and graphs: **[🔬 Launch in Simulator](../../../labs/advanced-laser-optics-lab.html#panel-michelson){target=_blank}**
...
A Michelson interferometer splits a coherent beam at a 50:50 beamsplitter into two arms, each terminated by a mirror. One mirror (M1) is mounted on a precision translation stage; the other (M2) is fixed. The two beams recombine at the beamsplitter and interfere, producing a pattern of circular fringes (in the collimated-beam, equal-inclination configuration) centred on the optical axis.

When the movable mirror M1 is displaced by ΔL, the round-trip path in that arm changes by 2ΔL, changing the optical path difference between the two arms by the same amount. Each time this path difference changes by exactly one wavelength λ, one complete ring collapses into (or emerges from) the centre of the pattern — i.e. the fringe order at the centre shifts by exactly 1.

By counting the number of fringes Δm that cross the centre while moving the mirror a known (or unknown) distance, the mirror displacement can be measured to a small fraction of a wavelength: ΔL = (Δm · λ)/2. This is the operating principle of the original Michelson–Morley experiment and of modern laser interferometric displacement and vibration sensors.
...
$$
\Delta m = \dfrac{2\,\Delta L}{\lambda}
$$

**Mirror displacement from fringe count:**

$$
\Delta L = \dfrac{\Delta m\,\lambda}{2}
$$

<div class="apparatus-3d-wrap" markdown="1">
<div class="apparatus-3d" data-setup="michelson-interferometer"></div>

<div class="apparatus-photo">
<img src="../../assets/apparatus/michelson-interferometer.png" alt="Apparatus layout diagram for this experiment">
...
<div class="apparatus-3d-caption"><b>3-D apparatus schematic</b> &middot; drag to rotate, scroll to zoom &middot; Laser &rarr; beamsplitter &rarr; fixed &amp; movable mirrors &rarr; detector</div>
</div>
...
|---|---|---|---|
| Movable mirror displacement | ΔL | 0 – 3000 | nm |
| Wavelength | λ | 400 – 700 | nm |

## Procedure

1. Set λ to a fixed value (e.g. 633 nm, He-Ne) and record the initial fringe order reading at ΔL = 0.
2. Increase ΔL in small, regular steps and record the optical path difference (2ΔL) and the fringe-order shift Δm displayed at each step.
3. Repeat the run for a second wavelength and compare the number of fringes crossing the centre for the same mechanical displacement.
4. Plot Δm against ΔL and verify the expected straight-line relationship of slope 2/λ.

## Observation Table

*Table: Fringe-order shift as a function of mirror displacement*

| S. No. | ΔL (nm) | λ (nm) | OPD = 2ΔL (nm) | Δm = 2ΔL/λ (observed) |
|---|---|---|---|---|
| 1 |  |  |  |  |
| 2 |  |  |  |  |
| 3 |  |  |  |  |
...
Plot **Fringe order shift Δm** (y-axis) against **Mirror displacement ΔL (nm)** (x-axis).

Expected trend: a straight line through the origin of slope 2/λ.

*Δm vs ΔL used to extract an experimental wavelength*
...
<text x="320.0" y="22" text-anchor="middle" font-size="17" font-weight="700" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">Fringe-Order Shift vs Mirror Displacement</text>
...
<text x="345.0" y="506" text-anchor="middle" font-size="14" fill="#142B66" font-weight="600" font-family="Inter,Helvetica,sans-serif">Mirror displacement, ΔL (nm)</text>
<text x="18" y="252.5" text-anchor="middle" font-size="14" fill="#142B66" font-weight="600" font-family="Inter,Helvetica,sans-serif" transform="rotate(-90 18 252.5)">Fringe order shift, Δm</text>
</svg>
</div>
...
1. From the slope of the Δm vs ΔL graph, compute λ_experimental = 2/slope.
2. Compare λ_experimental with the λ set on the simulator and compute the percentage error.

## Result

*Table: Wavelength determination by fringe counting*
...
ΔL = Δm·λ/2. The dominant error is usually miscounting fringes by ±1 near the start/end of a sweep, contributing ±λ/2 directly; the source's own spectral uncertainty adds a smaller relative term.

<div class="error-calc" data-exp="michelson-interferometer"></div>

## Precautions

- Move the mirror in one direction only during a counting run to avoid ambiguity from backlash.
- Choose a step size small enough that no fringe crossing is missed, especially at shorter wavelengths.

## Self-Check Quiz

??? question "1. Moving mirror M1 by ΔL changes the round-trip path in that arm by:"
    - **A)** ΔL
    - **B)** 2ΔL
    - **C)** ΔL/2
    - **D)** 4ΔL

    **Answer: B** — Light travels to M1 and back, so a mirror displacement ΔL changes the path length in that arm by 2ΔL — this is why the OPD = 2ΔL.


??? question "2. One complete ring collapses into (or emerges from) the centre of the pattern each time the OPD changes by:"
    - **A)** λ/4
    - **B)** λ/2
    - **C)** Exactly one wavelength λ
    - **D)** 2λ

    **Answer: C** — Each time the optical path difference changes by exactly one wavelength, the fringe order at the centre shifts by 1, i.e. one ring appears or disappears.


??? question "3. If you count Δm = 500 fringes while moving the mirror, using λ = 632.8 nm, the mirror displacement ΔL is closest to:"
    - **A)** 0.158 mm
    - **B)** 0.316 mm
    - **C)** 0.633 mm
    - **D)** 1.266 mm

    **Answer: A** — ΔL = Δm·λ/2 = 500 × 632.8 nm / 2 = 158,200 nm ≈ 0.158 mm.


??? question "4. A Michelson interferometer is fundamentally useful for precision metrology because it converts:"
    - **A)** Light intensity into colour
    - **B)** A tiny mechanical displacement into an easily-countable number of fringes
    - **C)** Coherent light into incoherent light
    - **D)** Wavelength into polarization angle

    **Answer: B** — Counting fringes as a mirror moves gives a displacement measurement accurate to a fraction of a wavelength — the basis of interferometric metrology.
...
3. Why does displacing the movable mirror by a distance λ/2 shift the fringe pattern by exactly one fringe?
