...
!!! abstract "Module"
    Geometrical Optics · Module 5


!!! tip "Try it live"
    Explore this on the interactive simulator with real-time sliders and graphs: **[🔬 Launch in Simulator](../../../labs/advanced-laser-optics-lab.html#panel-focusing){target=_blank}**
...
In the idealised ray (geometrical) picture, a convex lens brings parallel rays to a single point at its focal length f. In practice, however, no lens can focus light to a true mathematical point: the finite aperture D of the lens diffracts the beam, producing an Airy pattern whose central spot has a finite radius.

The ratio f/D, called the f-number, characterises how tightly a lens can focus light — a smaller f-number (relatively larger aperture) produces a smaller, more intense focal spot, right up to the diffraction limit. The radius of the Airy disc (first dark ring of the diffraction pattern) is given by 1.22 λ f/D, so shorter wavelengths and smaller f-numbers both produce tighter foci.

This diffraction limit sets the ultimate resolution of any imaging or focusing optical system, from microscope objectives to laser-machining heads, and is directly relevant to the industrial laser-processing workshops (drilling, cutting, welding) elsewhere in this lab, where a small focused spot is essential for high power density.
...
**Diffraction-limited (Airy) spot radius:**

$$
r_{\text{Airy}} \approx 1.22\,\dfrac{\lambda f}{D}
$$

<div class="apparatus-3d-wrap" markdown="1">
<div class="apparatus-3d" data-setup="convex-lens-focusing"></div>

<div class="apparatus-photo">
<img src="../../assets/apparatus/convex-lens-focusing.png" alt="Apparatus layout diagram for this experiment">
...
<div class="apparatus-3d-caption"><b>3-D apparatus schematic</b> &middot; drag to rotate, scroll to zoom &middot; Collimated beam &rarr; convex lens &rarr; focal screen</div>
</div>
...
|---|---|---|---|
| Focal length | f | 5 – 30 | cm |
| Aperture (lens diameter) | D | 1 – 10 | cm |
| Wavelength | λ | 400 – 700 | nm |

## Procedure

1. Set λ to a fixed value and vary the focal length f at constant aperture D; record the f-number and the Airy spot radius at each setting.
2. Reset f and vary the aperture D over its full range; record the f-number and spot radius.
3. Repeat for a second wavelength and compare the spot radii obtained.
4. Plot spot radius against f-number and verify the expected linear relationship.

## Observation Table

*Table: Diffraction-limited spot radius as a function of f-number*

| S. No. | f (cm) | D (cm) | f-number = f/D | λ (nm) | Spot radius 1.22λf/D (μm) |
|---|---|---|---|---|---|
| 1 |  |  |  |  |  |
| 2 |  |  |  |  |  |
| 3 |  |  |  |  |  |
...
Plot **Airy spot radius (μm)** (y-axis) against **f-number (f/D)** (x-axis).

Expected trend: a straight line through the origin of slope 1.22λ.

*Spot radius vs f-number at fixed wavelength*
...
<text x="320.0" y="22" text-anchor="middle" font-size="17" font-weight="700" fill="#0B173A" font-family="Inter,Helvetica,sans-serif">Spot Radius vs f-number</text>
...
<text x="345.0" y="506" text-anchor="middle" font-size="14" fill="#142B66" font-weight="600" font-family="Inter,Helvetica,sans-serif">f-number, N_f = f/D</text>
<text x="18" y="252.5" text-anchor="middle" font-size="14" fill="#142B66" font-weight="600" font-family="Inter,Helvetica,sans-serif" transform="rotate(-90 18 252.5)">Spot radius, r (μm)</text>
</svg>
</div>
...
1. For each trial compute the f-number and the theoretical Airy radius, and compare with the simulator readout.
2. From the slope of the spot-radius vs f-number graph, extract an experimental wavelength (slope = 1.22λ) and compare with the set value.

## Result

*Table: Wavelength check from diffraction-limited focusing*
...
r = 1.22λf/D is a product/quotient of three measured quantities — sum the fractional (least count / value) errors of each.

<div class="error-calc" data-exp="convex-lens-focusing"></div>

## Precautions

- Remember that a smaller f-number always gives a tighter spot, but only up to the diffraction limit — geometry alone never predicts a zero-size point.
- Keep wavelength fixed while studying the f versus D dependence, and vice versa.

## Self-Check Quiz

??? question "1. A lens with f = 100 mm and D = 50 mm has an f-number of:"
    - **A)** 0.5
    - **B)** 1
    - **C)** 2
    - **D)** 5000

    **Answer: C** — N_f = f/D = 100/50 = 2 (often written f/2).


??? question "2. Reducing the f-number (relatively larger aperture, same f) has what effect on the diffraction-limited spot size?"
    - **A)** Makes it larger
    - **B)** Makes it smaller (tighter focus)
    - **C)** No effect
    - **D)** Makes the spot disappear

    **Answer: B** — r_Airy ∝ f/D = N_f, so a smaller f-number gives a smaller Airy spot radius, i.e. a tighter focus.


??? question "3. Why can't any real lens focus light to a true mathematical point, even with a perfect, aberration-free design?"
    - **A)** All glass absorbs some light
    - **B)** The finite aperture diffracts the beam, producing an Airy pattern of finite size
    - **C)** Lenses always have chromatic aberration
    - **D)** Only laser light can be focused to a point

    **Answer: B** — Diffraction from the finite aperture D is a fundamental physical limit (not a manufacturing defect), setting the Airy-disk floor on spot size.


??? question "4. Comparing two wavelengths at the same f-number, which one focuses to the smaller spot?"
    - **A)** The longer wavelength
    - **B)** The shorter wavelength
    - **C)** Both are identical
    - **D)** Wavelength has no effect on spot size

    **Answer: B** — r_Airy ≈ 1.22 λf/D is directly proportional to λ, so shorter wavelengths focus to smaller diffraction-limited spots.
